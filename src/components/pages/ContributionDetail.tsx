      ...
      <ContentDetailLayout
        ...
        main={
          <section
            ...
            className="space-y-5 rounded-2xl bg-white/80 p-6 shadow-sm ring-1 ring-purple-100"
          >
            <h2
            ...
            {allMetadata.length > 0 && (
              <dl className="grid grid-cols-1 gap-4 rounded-2xl bg-white/70 p-4 shadow-inner ring-1 ring-purple-100 md:grid-cols-2">
                {allMetadata.map(item => (
                  <div key={item.label}>
                    <dt className="text-xs uppercase tracking-wide text-gray-500">{item.label}</dt>
                    <dd
                      className={`mt-1 text-base font-medium ${item.emphasis ? 'text-purple-900' : 'text-gray-700'}`}
                    >
                      {item.value}
                    </dd>
                  </div>
