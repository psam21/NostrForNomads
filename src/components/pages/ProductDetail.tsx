      ...
      <ContentDetailLayout
        media={
          <>
            ...
            {mediaItems.length > 0 && mediaItems.some(item => item.source?.size || item.source?.hash) && (
              <div className="mt-4 rounded-lg bg-white/80 p-4 shadow-sm ring-1 ring-gray-200">
                <h3 className="text-sm font-semibold text-gray-700 mb-3">Media Details</h3>
                <div className="space-y-3">
                  {mediaItems.map((item, index) => {
                    const source = item.source;
                    if (!source) return null;
                    
                    const hasMetadata = source.size || source.hash || source.dimensions;
                    if (!hasMetadata) return null;
                    
                    return (
                      <div key={item.id} className="text-xs border-t border-gray-200 pt-3 first:border-0 first:pt-0">
                        ...
                        <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
                          ...
                          {source.mimeType && (
                            <>
                              ...
                              <dd className="text-gray-900 font-mono">{source.mimeType}</dd>
                            </>
                          )}
                          {source.hash && (
                            <>
                              ...
                              <dd className="text-gray-900 font-mono text-xs">
                                <button
                                  onClick={() => {
                                    navigator.clipboard.writeText(source.hash || '');
                                    alert('Hash copied to clipboard!');
                                  }}
                                  className="hover:text-purple-600 truncate block max-w-full"
                                  title="Click to copy full hash"
                                >
                                  {source.hash.substring(0, 16)}...
                                </button>
                              </dd>
                            </>
                          )}
